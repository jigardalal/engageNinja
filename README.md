...
- Admin:
  - `scripts/ui/admin-global-tags.js`: platform admin creates/lists global tags
  - `scripts/ui/admin-tenant-edit.js`: admin edits tenant fields
  - `scripts/ui/admin-sync-tags.js`: sync global tags into a tenant
  - `scripts/ui/admin-audit-logs.js`: filters audit log viewer
  - `scripts/ui/admin-dashboard-summary.js`: checks dashboard stats cards
  - `scripts/ui/admin-tenant-banner.js`: verifies tenant context banner for platform admin
- Tenant:
  - `scripts/ui/tenant-tags.js`: tenant admin CRUD + member gating for tags
  - `scripts/ui/profile-edit.js`: user updates profile fields
- `scripts/ui/whatsapp-campaign.js`: creates a WhatsApp campaign with variables/media header flow.
- `scripts/ui/email-campaign.js`: creates an email campaign draft.
- `scripts/ui/settings-templates.js`: opens Settings → Channels and optionally syncs templates if WhatsApp is connected.
- `scripts/ui/smoke.js`: login → dashboard → contacts → campaigns smoke check.
- `scripts/ui/contacts-crud.js`: creates, edits, and deletes a contact.

Prereqs: Chrome/Chromium available (set `CHROME_PATH` if not at a default path).

By default, UI scripts assume the app is running on:
- Frontend: `http://localhost:3173`
- Backend: `http://localhost:5173`

To run UI automation against a different environment, set:
- `BASE_URL` (example: `BASE_URL=http://localhost:3174`)

Run via npm scripts:
```bash
npm run ui:test:whatsapp
npm run ui:test:email
npm run ui:test:settings
npm run ui:test:smoke
npm run ui:test:contacts
npm run ui:test:all
```
...
node scripts/ui/email-campaign.js
node scripts/ui/settings-templates.js
node scripts/ui/admin-global-tags.js
node scripts/ui/tenant-tags.js
node scripts/ui/admin-sync-tags.js
node scripts/ui/admin-audit-logs.js
node scripts/ui/admin-dashboard-summary.js
node scripts/ui/admin-tenant-banner.js
```

## Development Workflow

### Isolated Test Stack (separate DB + ports)
...
BASE_URL=http://localhost:3174 npm run ui:test:smoke
```
