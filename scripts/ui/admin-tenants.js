/**
 * UI test: Admin Tenants list/search/detail
 * - Platform admin sees tenant list
 * - Can search by name
 * - Can open tenant detail view
 */
...
async function run() {
  ...
  try {
    ...
    // Wait for tenants table
    await page.waitForSelector('table tbody tr', { timeout: 10000 });

    // Search for Demo tenant
    await page.type('input[placeholder="Search by name or ID..."]', 'Demo');
    await page.waitForTimeout(500);
    const rows = await page.$$eval('table tbody tr', (r) => r.length);
    if (rows < 1) {
      throw new Error('No tenant rows found after search');
    }
    ...
    if (!detailPath.startsWith('/admin/tenants/')) {
    ...
    await page.screenshot({ path: path.join(LOG_DIR, 'admin-tenants.png'), fullPage: true });
    console.log('✅ Admin tenants list/search/detail verified.');
  } catch (err) {
    console.error('❌ Admin tenants test failed:', err.message);
    process.exitCode = 1;
  } finally {
    await browser.close();
  }
