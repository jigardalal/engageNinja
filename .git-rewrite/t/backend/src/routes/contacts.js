  ...
  try {
    ...
    if (Array.isArray(tags) && tags.length > 0) {
      for (const tagId of tags) {
        ...
        const tag = db.prepare('SELECT id FROM tags WHERE id = ? AND tenant_id = ?').get(tagId, req.tenantId);
        if (tag) {
 ...
 */
router.post('/import', requireAuth, validateTenantAccess, (req, res) => {
  try {
    ...
    const insertContactTag = db.prepare('INSERT INTO contact_tags (contact_id, tag_id) VALUES (?, ?)');
    const getOrCreateTag = db.prepare('SELECT id FROM tags WHERE tenant_id = ? AND name = ?');
    const createTag = db.prepare('INSERT INTO tags (id, tenant_id, name, created_at, updated_at) VALUES (?, ?, ?, ?, ?)');
        ...
        try {
          ...
          if (tagsStr) {
            ...
            for (const tagName of tagNames) {
              let tag = getOrCreateTag.get(req.tenantId, tagName);
              if (!tag) {
                const tagId = uuidv4();
