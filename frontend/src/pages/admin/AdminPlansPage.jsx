...
export function AdminPlansPage() {
  ...
  return (
    <AppShell title="Platform Plans" subtitle="Edit pricing tiers and quota limits">
            ...
            {plans.map((plan) => (
              <Card key={plan.id}>
                  ...
                  {planMessages[plan.id]?.text && (
                    <p
                      className={`mt-2 text-sm ${
                        planMessages[plan.id].type === 'success' ? 'text-green-600' : 'text-red-600'
                      }`}
                    >
                      {planMessages[plan.id].text}
                    </p>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <p className="text-xs font-semibold text-[var(--text-muted)]">Team members</p>
                      <Input
                        type="number"
                        min={0}
                        step={1}
