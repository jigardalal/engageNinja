...
try {
  ...
  const invitationsCols = db.prepare(`PRAGMA table_info(user_invitations)`).all();
  const invitationsColNames = invitationsCols.map(c => c.name);
  ...
  const userTenantsCols = db.prepare(`PRAGMA table_info(user_tenants)`).all();
  const userTenantsColNames = userTenantsCols.map(c => c.name);
  ...
  const owner = db.prepare('SELECT COUNT(*) as count FROM users WHERE email = ?').get('admin@engageninja.local');
  check('Owner user exists (admin@engageninja.local)', owner.count > 0);

  const admin = db.prepare('SELECT COUNT(*) as count FROM users WHERE email = ?').get('user@engageninja.local');
  check('Admin user exists (user@engageninja.local)', admin.count > 0);

  const member = db.prepare('SELECT COUNT(*) as count FROM users WHERE email = ?').get('member@engageninja.local');
  check('Member user exists (member@engageninja.local)', member.count > 0);

  const viewer = db.prepare('SELECT COUNT(*) as count FROM users WHERE email = ?').get('viewer@engageninja.local');
  ...
  const invitationCount = db.prepare('SELECT COUNT(*) as count FROM user_invitations').get();
  info(`Total invitations in database: ${invitationCount.count}`);

  if (invitationCount.count > 0) {
    const sampleInvitation = db.prepare('SELECT * FROM user_invitations LIMIT 1').get();
  ...
  const indexes = db.prepare(`
    ...
    WHERE type='index' AND tbl_name IN ('user_invitations', 'user_tenants')
  `).all();
  ...
  const multiTenantUsers = db.prepare(`
  ...
  const orphanedTenants = db.prepare(`
  ...
  const orphanedTenants2 = db.prepare(`
    SELECT COUNT(*) as count FROM user_tenants ut
  ...
  const tenantsWithoutOwner = db.prepare(`
